...
export default {
    ...
    mounted() {
        axios.get('/api/search?title=' + this.title)
            .then(response => {
        ...
        window.onscroll = () => {
            ...
            if (bottomOfWindow) {
                ...
                let query = Object.keys(params)
                    ...
                    .join('&');
                axios.get(`/api/search?title=${this.title}&${query}&offset=${this.offset}`)
                    .then(response => {
                        // this.manga = response.data.data;
                        // this data push
    ...
    <Content>
        <div id="content" class="md:container md:mx-auto md:px-4">
            <div class="flex flex-row justify-between items-center mt-10 px-5">
                <h1 class="md:text-3xl text-xl font-bold text-white">Search {{ title }}</h1>
